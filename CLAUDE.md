...
ステージング環境に対してPlaywright等で検証を行った場合は、検証完了後に必ずデータを
削除して空の状態（migration適用済み・データなし）に戻す（`d1_migrations`以外の全テーブルを
子→親の順でDELETE、本番環境には絶対に実行しない）。ローカルDocker上のD1は
`.wrangler/state` ごと `rm -rf` する従来の手順のままでよい。
