...
### 9.4 講師の講座一覧は自分に関わる講座のみ表示する

`can_teach=true` かつ管理者ではないユーザーの講座一覧画面は、全講座ではなく、自分が
membershipを持つ講座（自分が作った講座、または既に生徒として参加申請中/参加中の
別講座）のみを表示する（拡張、issue #49）。全く関わりの無い講座まで並ぶと運用上
紛らわしいため。ただし「講師が別の講座の生徒になる」機能自体は制限しない —
講座詳細ページ（URLを直接開いた場合）からの参加申請や、招待URL経由の参加は
引き続き可能で、絞り込みは一覧表示のみに適用する。管理者・生徒（can_teach=false）の
講座一覧は従来通り全講座を表示する（生徒は新しい講座を探して参加申請できる必要が
あるため）。
